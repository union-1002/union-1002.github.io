...
export default function Board({
  ...
  return (
    <div className="relative" ref={boardRef}>
      ...
      {/* 게시글 목록 */}
      <div className="max-w-2xl mx-auto flex flex-col items-center gap-6 mt-12 px-4">
        ...
        {posts.map((post) => (
          <div
            key={post.id}
            className="w-full bg-white/10 backdrop-blur-md rounded-2xl border border-white/20 shadow-lg px-6 py-6 text-white hover:bg-white/20 transition"
          >
            <PostItem
              boardId={boardId}
              post={post}
      ...
      <div className="flex items-center justify-center space-x-2 mt-6">
        <button
          ...
          disabled={page <= 1}
          className="px-3 py-1 rounded-md border text-sm select-none cursor-pointer disabled:cursor-default bg-white/10 text-white hover:bg-white/50 disabled:opacity-50 disabled:bg-white/10 border-white/20"
        >
          이전
        </button>
...
function PostItem({ boardId, post, update, currentCommandId, setCurrentCommandId, isRepliable, usernameModifiable }) {
  ...
  return (
    <div className="">
      <div onClick={handleReply} className={`${isRepliable && 'cursor-pointer '} group relative flex flex-wrap items-start text-sm py-2 transition`}>

        <div className="text-left lg:text-center font-medium shrink-0 text-gray-400">
          {editMode && isUsernameEditable
            ? <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="w-full" />
            : post.author
          }
        </div>
        <div className="w-full mt-5 whitespace-pre-line">
          {editMode && isTextEditable
            ? <div className="flex">
                <textarea
                  className="w-full min-h-14 block p-2 text-sm text-white border bg-white/10 border-gray-300 rounded-lg"
                  placeholder="내용을 입력하세요."
                  value={editingText}
                  onChange={(e) => setEditingText(e.target.value)}
                />
                <button onClick={handleEditSubmit} className="rounded mt-6 w-14 cursor-pointer  bg-white/10 hover:bg-white/20 font-medium text-sm px-2 py-1">수정</button>
              </div>
...
function PostWriter({ boardId, update, usernameModifiable }) {
  ...
  return (
    <form className="mb-6">
      ...
      <div className="flex">
        <textarea
          className="w-full min-h-9 block p-2 text-sm text-white border bg-white/10 border-white/20 rounded-lg"
          placeholder="내용을 입력하세요."
          value={text}
          onChange={(e) => setText(e.target.value)}
        />
        <button onClick={handleSubmit} className="rounded w-14 cursor-pointer text-sm text-white border bg-white/10 border-white/20 hover:bg-white/20 ml-3">등록</button>
      </div>
    </form>
  );
}
