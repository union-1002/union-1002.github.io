...
function MissionCard2({ className, title, subtitle, status, children }) {
  return (
    <div className={className}>
      {/* 제목 영역 */}
      <div className="text-center border-b-3 border-[#7640ee] mb-3"></div>
      <div className="text-center border-b-3 border-[#7640ee] mb-3"></div>
      <div className="text-center border-3 border-[#7640ee] py-6 bg-[#f2ebfd]">
        <div className="text-2xl font-bold">{title}</div>
        <div className="text-sm mt-1">{subtitle}</div>
      </div>
      {/* 내용 영역 */}
      <div className="bg-white border-l-3 border-r-3 border-b-3 py-6 px-4 border-[#7640ee]">
        {children}
      </div>
      <div className='flex justify-end'>
        <div className="w-fit bg-[#7640ee] text-white px-10 py-2">{status}</div>
      </div>
    </div>
  );
}

function MissionCard3({ className, title, subtitle, status, children }) {
...
<div className="relative overflow-visible">
  <div className="relative overflow-visible">
    <div
      className="absolute inset-0 bg-[#7640ee]"
      style={{
        clipPath: 'polygon(6rem 0%, calc(100% - 2rem) 0%, 100% 2rem, 100% 100%, 0% 100%, 0% 6rem)',
      }}
    ></div>
    <div className='p-0.5'>
      <div
...
<div className="relative overflow-visible">
  <div className="relative overflow-visible">
    ...
    <div
      className="relative bg-white py-6"
      style={{
        clipPath: `polygon(calc(6rem + ${storkeWidth2}) calc(0% + ${storkeWidth}), calc(100% - 2rem - ${storkeWidth2}) calc(0% + ${storkeWidth}), calc(100% - ${storkeWidth}) calc(2rem + ${storkeWidth2}), calc(100% - ${storkeWidth}) calc(100% - ${storkeWidth}), calc(0% + ${storkeWidth}) calc(100% - ${storkeWidth}), calc(0% + ${storkeWidth}) calc(6rem + ${storkeWidth2}))`,
      }}
    >
      <div className="text-2xl font-bold">{title}</div>
      <div className="text-sm mt-1">{subtitle}</div>
...
{/* 
      ...
      <div className="flex flex-row">
        <div className="flex flex-col">
          <div className="bg-red-500 w-15 h-15" />
          <div />
        </div>
        <div className="text-center border-3 border-[#7640ee] py-6 bg-[#f2ebfd]">
          <div className="text-2xl font-bold">{title}</div>
